...
public class vp_FPSDemo2 : MonoBehaviour
{
	...
	private float m_GoAgainButtonAlpha;

	private float m_WASDInfoClickTime;
	...
	private Vector3 m_BlownAwayPos = new Vector3(132f, 122.18f, -100.6f);

	private Vector3 m_ActionPos = new Vector3(127f, 122.18f, -97.6f);
	...
	private Vector3 m_ExplorePos = new Vector3(134.0023f, 107.642609f, -109.5f);

	private Vector3 m_OverViewPos = new Vector3(135f, 105.8f, -70.7f);

	private Vector3 m_OutroPos = new Vector3(135f, 205.8f, -70.7f);

	private Vector2 m_OutroAngle = new Vector2(-19.3f, 241.7f);

	private Vector2 m_SlimeAngle = new Vector2(0f, 180f);

	private Vector2 m_WetRoofAngle = new Vector2(30f, 230f);

	private Vector2 m_FallDeflectAngle = new Vector2(25f, 180f);

	private Vector2 m_BlownAwayAngle = new Vector2(0f, -90f);

	private Vector2 m_ActionAngle = new Vector2(0f, 180f);

	private Vector2 m_HeadBumpAngle = new Vector2(0f, 180f);

	private Vector2 m_WallBounceAngle = new Vector2(0f, 130f);

	private Vector2 m_ExploreAngle = new Vector2(30f, 40f);

	private Vector2 m_OverViewAngle = new Vector2(-16.5f, 215f);

	private vp_Timer.Handle m_ForceTimer = new vp_Timer.Handle();

	private vp_Timer.Handle m_GoAgainTimer = new vp_Timer.Handle();

	private vp_Timer.Handle m_HeadBumpTimer1 = new vp_Timer.Handle();

	private vp_Timer.Handle m_HeadBumpTimer2 = new vp_Timer.Handle();

	private vp_Timer.Handle m_ActionTimer1 = new vp_Timer.Handle();

	private vp_Timer.Handle m_ActionTimer2 = new vp_Timer.Handle();

	private vp_Timer.Handle m_ActionTimer3 = new vp_Timer.Handle();

	private vp_Timer.Handle m_ActionTimer4 = new vp_Timer.Handle();

	private vp_Timer.Handle m_ActionTimer5 = new vp_Timer.Handle();

	private float m_OutroStartTime;

	private bool m_RunForward;

	private bool m_Jump;

	private int m_LookPoint;
		...
		if (m_Demo.ButtonSelection != m_ExamplesCurrentSel)
		{
			...
			case 5:
				...
				vp_Timer.In(1f, delegate
				{
					...
					m_Demo.LookDamping = 3f;
					m_Demo.Controller.PhysicsWallBounce = 0f;
					Object.Instantiate(m_ExplosionFX, m_Demo.Controller.transform.position + new Vector3(3f, 0f, 0f), Quaternion.identity);
	...
	private void FallImpact(float f)
	{
		if (f > 0.2f)
		{
