...
public class vp_FractalNoise
{
	...
	public vp_FractalNoise(float inH, float inLacunarity, float inOctaves, vp_Perlin noise)
	{
		...
		{
			m_Exponent[i] = (float)Math.Pow(m_Lacunarity, 0.0 - (double)inH);
			num *= m_Lacunarity;
		}
		if (noise == null)
		{
	...
	public float BrownianMotion(float x, float y)
	{
		...
		{
			num = m_Noise.Noise(x, y) * m_Exponent[(int)(IntPtr)num2];
			x *= m_Lacunarity;
			y *= m_Lacunarity;
		}
		float num3 = m_Octaves - (float)m_IntOctaves;
		return num + num3 * m_Noise.Noise(x, y) * m_Exponent[(int)(IntPtr)num2];
	}
}
