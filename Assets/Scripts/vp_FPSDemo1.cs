...
public class vp_FPSDemo1 : MonoBehaviour
{
	...
	private int m_ExamplesCurrentSel;
	...
	private bool m_WeaponLayerToggle;
	...
	private Vector3 m_AstronautPos = new Vector3(20f, 20f, 16f);

	protected Vector3 m_UnFreezePosition = Vector3.zero;

	private Vector2 m_MouseLookAngle = new Vector2(0f, 33.10683f);

	private Vector2 m_OverviewAngle = new Vector2(28.89369f, 224f);

	private Vector2 m_PerspectiveAngle = new Vector2(27f, 223f);

	private Vector2 m_StartAngle = new Vector2(0f, 0f);

	private Vector2 m_WeaponLayerAngle = new Vector2(0f, -90f);

	private Vector2 m_ForcesAngle = new Vector2(-167f, 33.10683f);

	private Vector2 m_MechAngle = new Vector3(0f, 180f);

	private Vector2 m_DrunkAngle = new Vector3(0f, -90f);

	private Vector2 m_SniperAngle = new Vector2(20f, 180f);

	private Vector2 m_OldSchoolAngle = new Vector2(0f, 180f);

	private Vector2 m_AstronautAngle = new Vector2(0f, 269.5f);
	...
	private void Start()
	{
		...
		m_Demo.PlayerEventHandler.SetWeapon.Disallow(1E+07f);
		m_Demo.PlayerEventHandler.SetPrevWeapon.Try = (() => false);
		m_Demo.PlayerEventHandler.SetNextWeapon.Try = (() => false);
	}
	...
	private void SetWeapon(int i, string state = null, bool drawCrosshair = true, bool wieldMotion = true)
	{
		...
		if (m_Demo.WeaponHandler.CurrentWeaponIndex != i)
		{
			if (m_Demo.WeaponHandler.CurrentWeapon != null)
			{
				...
				{
					m_Demo.WeaponHandler.CurrentWeapon.Wield(showWeapon: false);
				}
			}
	...
	private void DemoExamples()
	{
		...
		if (m_Demo.ButtonSelection != m_ExamplesCurrentSel)
		{
			...
			{
			case 0:
				m_Demo.PlayerEventHandler.Attack.Stop(1E+07f);
				m_Demo.DrawCrosshair = true;
			...
			case 6:
				...
				m_Demo.Camera.BobStepCallback = delegate
				{
					...
					if (m_Demo.WeaponHandler.CurrentWeapon != null)
					{
						m_Demo.WeaponHandler.CurrentWeapon.AddForce(new Vector3(0f, 0f, 0f), new Vector3(-0.3f, 0f, 0f));
					}
	...
	private void DemoForces()
	{
		...
		if (m_Demo.ButtonSelection != -1)
		{
			...
			case 0:
				m_Demo.Camera.Load(StompingCamera);
				m_Demo.Controller.Load(SmackController);
				m_Demo.PlayerEventHandler.Earthquake.TryStart(new Vector3(0.2f, 0.2f, 10f));
			...
			case 1:
				...
				m_Demo.Controller.Load(SmackController);
				m_Demo.PlayerEventHandler.GroundStomp.Send(1f);
			...
			case 2:
			{
				...
				m_Demo.PlayerEventHandler.BombShake.Send(1f);
				...
				Vector3 position2 = m_Demo.Controller.transform.position;
				position.y = position2.y + 1f;
				Object.Instantiate(m_ArtilleryFX, position, Quaternion.identity);
				break;
			}
			case 3:
				m_Demo.Camera.Load(StompingCamera);
				m_Demo.Controller.Load(SmackController);
				m_Demo.PlayerEventHandler.Earthquake.TryStart(new Vector3(0.25f, 0.2f, 10f));
				m_Demo.ButtonColumnArrowFadeoutTime = Time.time + 9f;
				m_AudioSource.Stop();
				m_AudioSource.pitch = Time.timeScale;
				m_AudioSource.PlayOneShot(m_EarthquakeSound);
	...
	private void DemoMouseInput()
	{
		...
		if (m_Demo.FirstFrame)
		{
			...
			m_AudioSource.Stop();
			m_Demo.DrawCrosshair = true;
			m_Demo.FreezePlayer(m_MouseLookPos, m_MouseLookAngle, freezeCamera: true);
		...
		m_Demo.Camera.MouseAcceleration = m_Demo.ButtonToggle(new Rect(Screen.width / 2 + 110, 215f, 90f, 40f), "Acceleration", m_Demo.Camera.MouseAcceleration, arrow, m_ImageUpPointer);
		GUI.color = new Color(1f, 1f, 1f, 1f * m_Demo.GlobalAlpha);
		GUI.enabled = true;
	...
	private void DemoWeaponPerspective()
	{
		...
		if (m_Demo.FirstFrame)
		{
			...
			m_Demo.WeaponHandler.SetWeaponLayer(31);
			if (m_Demo.WeaponHandler.CurrentWeapon != null)
			{
				m_Demo.WeaponHandler.CurrentWeapon.SnapZoom();
				m_Demo.WeaponHandler.CurrentWeapon.SnapSprings();
	...
	private void DemoWeaponLayer()
	{
		...
		if (m_Demo.FirstFrame)
		{
			...
			int weaponLayer = m_WeaponLayerToggle ? 31 : 0;
		...
		if (m_Demo.ShowGUI)
		{
			...
			{
				m_Demo.FreezePlayer(m_WeaponLayerPos, m_WeaponLayerAngle);
				int weaponLayer2 = m_WeaponLayerToggle ? 31 : 0;
	...
	private void DemoPivot()
	{
		...
		if (m_Demo.FirstFrame)
		{
			...
			if (m_Demo.WeaponHandler.CurrentWeapon != null)
			{
				m_Demo.WeaponHandler.CurrentWeapon.SetPivotVisible(visible: true);
			}
			m_Demo.Input.ForceCursor = true;
			m_Demo.WeaponHandler.SetWeaponLayer(31);
		}
