...
public class vp_SmoothRandom
{
	...
	public static Vector3 GetVector3Centered(float speed)
	{
		...
		Vector3 a = new Vector3(Get().HybridMultifractal(x, 15.73f, 0.58f), Get().HybridMultifractal(x, 63.94f, 0.58f), Get().HybridMultifractal(x, 0.2f, 0.58f));
		Vector3 b = new Vector3(Get().HybridMultifractal(x2, 15.73f, 0.58f), Get().HybridMultifractal(x2, 63.94f, 0.58f), Get().HybridMultifractal(x2, 0.2f, 0.58f));
		return a - b;
	}
