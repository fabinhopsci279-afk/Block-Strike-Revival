...
{
	public string OnGrabText = string.Empty;
	...
	protected virtual void UpdatePosition()
	{
		...
		Vector3 currentSwayForce = m_CurrentSwayForce;
		Transform transform = m_Camera.Transform;
		float x = m_CurrentMouseMove.x * 0.05f;
		Vector2 vector = m_Player.Rotation.Get();
		m_CurrentSwayForce = currentSwayForce + transform.TransformDirection(new Vector3(x, (!(vector.x <= m_Camera.RotationPitchLimit.y)) ? (m_CurrentMouseMove.y * 0.015f) : (m_CurrentMouseMove.y * 0.05f), 0f));
		Transform transform2 = m_Transform;
		Vector3 position = m_Transform.position;
		Vector3 a = m_Camera.Transform.position - m_CurrentSwayForce + m_Camera.Transform.right * CarryingOffset.x;
		Vector3 up = m_Camera.Transform.up;
		Vector3 localScale = m_Transform.localScale;
		transform2.position = Vector3.Lerp(position, a + up * localScale.y * (CarryingOffset.y + m_CurrentShake.y * 0.5f) + m_Camera.Transform.forward * CarryingOffset.z, (!(m_FetchProgress >= 1f)) ? m_FetchProgress : (Time.deltaTime * (Stiffness * 60f)));
	}
	...
	protected virtual void OnCollisionStay(Collision col)
	{
		...
		if (Physics.Raycast(m_Transform.position, m_Camera.Transform.forward, out RaycastHit hitInfo, 1f) && hitInfo.collider == m_LastExternalCollider && m_FetchProgress >= 1f)
		{
