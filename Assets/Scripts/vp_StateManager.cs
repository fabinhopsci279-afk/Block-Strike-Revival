...
public class vp_StateManager
{
	...
	public vp_StateManager(vp_Component component, List<vp_State> states)
	{
		...
		foreach (vp_State state in m_States)
		{
			...
			{
				UnityEngine.Debug.LogWarning("Warning: " + m_Component.GetType() + " on '" + m_Component.name + "' has more than one state named: '" + state.Name + "'. Only the topmost one will be used.");
	...
	public void ImposeBlockingList(vp_State blocker)
	{
		...
		{
			for (int i = 0; i < blocker.StatesToBlock.Count; i++)
			{
				m_States[blocker.StatesToBlock[i]].AddBlocker(blocker);
			}
		}
	}
	...
	public void SetState(string state, bool setEnabled = true)
	{
		if (AppPlaying() && m_StateIds.TryGetValue(state, out m_TargetId))
		{
			...
			{
				UnityEngine.Debug.LogWarning("Warning: The 'Default' state cannot be disabled.");
				return;
			}
