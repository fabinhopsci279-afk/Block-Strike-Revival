...
public class vp_Input : MonoBehaviour
{
		...
		get
		{
			if (mIsDirty)
			{
				...
				if (m_Instance == null)
				{
					...
					m_Instance.SetupDefaults(string.Empty);
				}
			}
			return m_Instance;
		}
	...
	{
		if ((type == string.Empty || type == "Buttons") && ButtonKeys.Count == 0)
		{
		...
		if ((type == string.Empty || type == "Axis") && AxisKeys.Count == 0)
		{
			AddAxis("Vertical", KeyCode.W, KeyCode.S);
			AddAxis("Horizontal", KeyCode.D, KeyCode.A);
		}
		if ((type == string.Empty || type == "UnityAxis") && UnityAxis.Count == 0)
		{
	...
	public virtual bool DoGetButtonAny(string button)
	{
		...
		{
			return UnityEngine.Input.GetKey(Buttons[button]) || UnityEngine.Input.GetKeyDown(Buttons[button]) || UnityEngine.Input.GetKeyUp(Buttons[button]);
		}
		UnityEngine.Debug.LogError("\"" + button + "\" is not in VP Input Manager's Buttons. You must add it for this Button to work.");
		return false;
	}
	...
	public virtual bool DoGetButton(string button)
	{
		if (Buttons.ContainsKey(button))
		{
			return UnityEngine.Input.GetKey(Buttons[button]);
		}
		UnityEngine.Debug.LogError("\"" + button + "\" is not in VP Input Manager's Buttons. You must add it for this Button to work.");
		return false;
	}
	...
	public virtual bool DoGetButtonDown(string button)
	{
		if (Buttons.ContainsKey(button))
		{
			return UnityEngine.Input.GetKeyDown(Buttons[button]);
		}
		UnityEngine.Debug.LogError("\"" + button + "\" is not in VP Input Manager's Buttons. You must add it for this Button to work.");
		return false;
	}
	...
	public virtual bool DoGetButtonUp(string button)
	{
		if (Buttons.ContainsKey(button))
		{
			return UnityEngine.Input.GetKeyUp(Buttons[button]);
		}
		UnityEngine.Debug.LogError("\"" + button + "\" is not in VP Input Manager's Buttons. You must add it for this Button to work.");
		return false;
	}
	...
	public virtual float DoGetAxisRaw(string axis)
	{
		...
		{
			float result = 0f;
			if (UnityEngine.Input.GetKey(Axis[axis].Positive))
			{
				result = 1f;
			}
			if (UnityEngine.Input.GetKey(Axis[axis].Negative))
			{
				result = -1f;
			}
			return result;
		...
		UnityEngine.Debug.LogError("\"" + axis + "\" is not in VP Input Manager's Unity Axis. You must add it for this Axis to work.");
		return 0f;
	}
	...
	public static void ChangeButtonKey(string button, KeyCode keyCode, bool save = false)
	{
		...
		{
			UnityEngine.Debug.LogWarning("The Button \"" + button + "\" Doesn't Exist");
			return;
		}
		if (save)
		{
