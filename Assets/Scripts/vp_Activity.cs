...
public class vp_Activity : vp_Event
{
	...
	public float MinDuration
	{
		...
		set
		{
			...
			if (m_MaxDuration != -1f && m_MinDuration > m_MaxDuration)
			{
				...
				UnityEngine.Debug.LogWarning("Warning: (vp_Activity) Tried to set MinDuration longer than MaxDuration for '" + base.EventName + "'. Capping at MaxDuration.");
			}
		}
	}
	...
	public float AutoDuration
	{
		...
		set
		{
			...
			if (m_MaxDuration < m_MinDuration)
			{
				...
				UnityEngine.Debug.LogWarning("Warning: (vp_Activity) Tried to set MaxDuration shorter than MinDuration for '" + base.EventName + "'. Capping at MinDuration.");
			}
		}
	}
	...
	public object Argument
	{
		get
		{
			...
			{
				UnityEngine.Debug.LogError("Error: (" + this + ") Tried to fetch argument from '" + base.EventName + "' but this activity takes no parameters.");
				return null;
			}
			return m_Argument;
		}
		set
		{
			if (m_ArgumentType == null)
			{
				UnityEngine.Debug.LogError("Error: (" + this + ") Tried to set argument for '" + base.EventName + "' but this activity takes no parameters.");
			}
			else
			{
				m_Argument = value;
	...
	public bool TryStart(bool startIfAllowed = true)
	{
		...
		{
			Condition condition = (Condition)invocationList[i];
			if (!condition())
			{
				m_Argument = null;
				if (startIfAllowed)
