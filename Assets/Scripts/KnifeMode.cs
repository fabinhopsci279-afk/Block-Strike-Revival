...
public class KnifeMode : Photon.MonoBehaviour
{
	...
	private void Start()
	{
		...
		UIGameManager.SetActiveScore(active: true, 80);
		WeaponManager.SetPistolType(0);
		WeaponManager.SetRifleType(0);
		UISelectTeam.OnStart();
		GameManager.SetMode(GameMode.KnifeMode);
	...
	private void OnKilledPlayer(string info)
	{
		...
		if (AdsManager.isLoadedAds)
		{
			...
			{
				PlayerLevelManager.UpdatePlayerXP(7);
				SaveLoadManager.SetMoney1(7);
				SaveLoadManager.SetHeadshot1();
			}
			else
			{
				PlayerLevelManager.UpdatePlayerXP(3);
