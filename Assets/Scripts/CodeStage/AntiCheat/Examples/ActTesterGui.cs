		...
		private void OnGUI()
		{
						...
						using (new VerticalLayout())
						{
							...
							using (new HorizontalLayout())
							{
								...
								using (new VerticalLayout())
								{
									...
									GUILayout.Space(5f);
									GUILayout.Label("<color=\"" + (savesAlterationDetected ? "#FF4040" : "#02C85F") + "\">Saves modification detected: " + savesAlterationDetected + "</color>");
									GUILayout.Label("<color=\"" + (foreignSavesDetected ? "#FF4040" : "#02C85F") + "\">Foreign saves detected: " + foreignSavesDetected + "</color>");
								}
							}
						}
						GUILayout.Space(5f);
			...
			else
			{
				...
				using (new VerticalLayout(GUI.skin.box))
				{
					...
					GUILayout.Label("<color=\"" + (speedHackDetected ? "#FF4040" : "#02C85F") + "\">Detected: " + speedHackDetected.ToString().ToLower() + "</color>");
					GUILayout.Space(10f);
					...
					GUILayout.Label("<color=\"" + (obscuredTypeCheatDetected ? "#FF4040" : "#02C85F") + "\">Detected: " + obscuredTypeCheatDetected.ToString().ToLower() + "</color>");
					GUILayout.Space(10f);
					...
					GUILayout.Label("<color=\"" + (wallHackCheatDetected ? "#FF4040" : "#02C85F") + "\">Detected: " + wallHackCheatDetected.ToString().ToLower() + "</color>");
					GUILayout.Space(10f);
					GUILayout.Label("<b>Injection Detector</b>");
					GUILayout.Label("Allows to detect foreign managed assemblies in your application.");
					GUILayout.Label("<color=\"" + (injectionDetected ? "#FF4040" : "#02C85F") + "\">Detected: " + injectionDetected.ToString().ToLower() + "</color>");
				}
			}
			GUILayout.EndArea();
		}
