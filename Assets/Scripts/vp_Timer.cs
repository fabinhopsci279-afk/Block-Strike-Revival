...
public class vp_Timer : MonoBehaviour
{
	...
	private class Event
	{
		...
		{
			string message2 = "Error: (" + this + ") " + message;
	...
	public class Handle
	{
		...
		public bool CancelOnLoad
		{
			get
			{
				return !Active || m_Event.CancelOnLoad;
			}
			set
			{
				if (Active)
	...
	public bool WasAddedCorrectly => Application.isPlaying && !(base.gameObject != m_MainObject);
	...
	private void Update()
	{
		...
		while (true)
		{
			if (m_Active.Count > 0 && m_EventBatch < MaxEventsPerFrame)
			{
