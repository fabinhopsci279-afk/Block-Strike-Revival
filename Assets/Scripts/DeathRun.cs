...
public class DeathRun : Photon.MonoBehaviour
{
	...
	private void Start()
	{
		UIGameManager.SetActiveScore(active: true, GameManager.MaxScore);
		GameManager.SetStartDamageTime(1f);
		UIPanelManager.ShowPanel("Display");
		GameManager.SetMode(GameMode.DeathRun);
	...
	private void OnCreatePlayer()
	{
		...
		float speed = (GameManager.GetPlayerTeam() != Team.Blue) ? 0.25f : 0.2f;
		playerInput.UpdatePlayerSpeed(speed);
	}
