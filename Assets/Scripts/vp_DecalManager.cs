...
public sealed class vp_DecalManager
{
	...
	public static float FadedDecals
	{
		...
		set
		{
			...
			{
				UnityEngine.Debug.LogError("FadedDecals can't be larger than MaxDecals");
				return;
			}
			m_FadedDecals = value;
			Refresh();
	...
	private static void FadeAndRemove()
	{
		...
		if (m_Decals[0] != null)
		{
			Color color2 = m_Decals[0].GetComponent<Renderer>().material.color;
			if (color2.a <= 0f)
			{
	...
	private static void DebugOutput()
	{
		...
		UnityEngine.Debug.Log("Decal count: " + m_Decals.Count + ", Full: " + num + ", Faded: " + num2);
	}
}
